...
O site possui modo publico sem login e modo interno com Google Firebase Auth. Sem login, ou com usuario nao autorizado, a pagina mostra clima por CEP, cidade brasileira ou localizacao do navegador usando APIs externas. Somente `anvmano@gmail.com` e `clarissamikado@gmail.com` acessam o dashboard interno completo, listeners Firebase internos, exportacao interna e assistente IA.
...
- `scripts/views/public-weather-view.js`: modo publico por CEP/cidade/localizacao, selecao de cidades homonimas, cards, graficos externos e recomendacoes ambientais.
...
- Swipe touch segue o fluxo Estacao ⇄ Sala ⇄ Quarto ⇄ Aquario. Arrastar para esquerda avanca; arrastar para direita volta; extremidades nao mudam de aba. Gestos iniciados em tabelas, heatmaps ou qualquer area com rolagem horizontal nao trocam de aba. Em celular em paisagem, as quatro abas continuam na largura total, enquanto a toolbar de data/exportacao fica em uma linha propria.
...
- A toolbar mantem somente os controles globais de data e exportacao. Os rotulos redundantes `Agora (DD/MM)` e `Data consultada: DD/MM/AAAA` nao sao exibidos na barra, na faixa das estacoes nem nos cards globais; o calendario continua sendo a fonte visivel da data consultada.
- Cards omitem o bloco de qualidade quando a metrica esta adequada e com 100% de cobertura. Estado, amostras, cobertura e ultima leitura aparecem apenas quando houver ausencia, cobertura inferior a 100%, desatualizacao ou leitura suspeita/critica.
- O header exibe os chips na ordem Estacao do ano, AQI, ciclo solar, fase da lua e relogio. Estacao do ano, ciclo solar e lua usam icone/estado visual; descricoes ficam no `title`/`aria-label` e popover. No mobile, AQI tambem mostra prefixo e numero sobre fundo de contraste. Relogio e marca podem ser ocultados; os quatro chips principais ocupam a largura util.
- Indicador astronomico do header e um chip visual no tamanho aproximado do relogio, sem texto interno; o tooltip/`aria-label` mostra nascer e por do sol. Clique/toque abre popover com amanhecer, nascer do sol, zenite, por do sol, anoitecer, estado atual e duracao do dia. No modo interno usa `historico/NascePorDoSol`; no modo publico usa os eventos solares da localizacao consultada por CEP, cidade ou navegador pela Open-Meteo. Antes de uma localizacao publica ser consultada, deve ficar em estado aguardando e orientar os tres meios de consulta, sem usar fallback interno como se fosse dado real.
- Indicador AQI tem dois modos: internamente usa as ultimas leituras de Sala/MQ135 (`historico/AirQuality`) e se identifica como `AQI estimado da Sala`; publicamente usa a consulta externa, sem mencionar Sala/MQ135. O mobile exibe o numero dentro do chip; tooltip/`aria-label` e popover conservam valor e classificacao. O mini medidor usa ponteiro derivado da classificacao AQI.
...
- Chat responde comparacoes solares: duracao do dia, maior/menor duracao de luz no ano da data selecionada por padrao, no mes quando um mes for informado ou no intervalo completo quando a pergunta trouxer duas datas explicitas. Intervalos solares de ate um ano preservam as datas inicial e final e nao podem ser convertidos em comparacao comum entre dois dias.
- Resultados solares analiticos enviados ao redator da IA usam resumo compacto com totais e ranking, sem listas de centenas de datas que possam truncar o resultado principal.
...
- APIs externas nao substituem Firebase interno: atendem o modo publico por CEP/cidade/localizacao e, sob acao explicita do usuario, complementam a aba Estacao interna com chuva, UV e condicoes externas para a recomendacao de ventilacao.
